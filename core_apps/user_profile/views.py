...
from .serializers import NextOfKinSerializer, ProfileSerializer
...
class ProfileFilter(FilterSet):
    ...
    id_no = CharFilter(field_name="user__id_no")
...
class ProfileListApiView(generics.ListAPIView):
    ...
    search_fields = ["user__id_no", "user__first_name", "user__last_name"]

    def get_queryset(self) -> List[Profile]:
        return Profile.objects.exclude(user__is_superuser=True).exclude(
            user__is_staff=True
        )
            ...
            with transaction.atomic():
                ...
                if updated_instance.is_complete_with_next_of_kin():
                    ...
                    if not existing_account:
                        ...
                        message = (
                            "Profile updated successfully and a new bank account has been created. "
                            "An email with further instructions has been sent to you"
                        )
                    else:
                        message = (
                            "Profile updated successfully. "
                            "No new account created as one already exist for this currency and type"
                        )
                    return Response(
                    ...
                    return Response(
                        {
                            "message": "Profile updated successfully. Please complete you profile "
                            "with all required fields and at least one next of kin to create"
                            " a bank account",
...
class NextOfKinListApiView(generics.ListCreateAPIView):
    ...
    def get_serializer_context(
        self,
    ) -> dict[str, Any]:  # Add a user profile to the serializer context
...
class NextOfKinDetailApiView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def destroy(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        return Response(
            {"message": "Next of Kin delete successfully"},
