...
class ProfileSerializer(serializers.ModelSerializer):
    ...
    phone_number = PhoneNumberField()
    next_of_kin = NextOfKinSerializer(read_only=True, many=True)
    ...
    def validate(self, attrs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if id_issue_date and id_expiry_date and id_issue_date > id_expiry_date:
            raise serializers.ValidationError(
                {
                    "id_expiry_date": "ID or Passport Expiry Date must be after ID or Passport Issue Date"
                }
            )
        return attrs
    ...
    def update(self, instance: Profile, validated_data: dict) -> Profile:
        ...
        for field in ["photo", "id_photo", "signature_photo"]:
            if field in validated_data:
                photo_data = validated_data.pop(field)
                if photo_data.size > settings.MAX_UPLOAD_SIZE:
                    temp_file = default_storage.save(
                        f"{field}_{instance.id}.jpg", ContentFile(photo_data.read())
                    )
