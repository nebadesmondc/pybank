...
class Profile(TimeStampedModel):
    ...
    account_currency = models.CharField(
        ...
        default=BankAccount.AccountCurrency.XAF,
        null=True,
        blank=True,
    )
    ...
    id_photo = CloudinaryField(
        _("ID or Passport Photo"),
        blank=True,
        null=True,
    )
    id_photo_url = models.URLField(
        _("ID or Passport Photo URL"),
        blank=True,
        null=True,
    )
        ...
        if self.id_issue_date and self.id_expiry_date:
            if self.id_issue_date > self.id_expiry_date:
                raise ValidationError(
                    {
                        "id_expiry_date": _(
                            "ID or Passport Expiry Date must be after ID or Passport Issue Date"
                        )
                    }
                )
...
class NextOfKin(TimeStampedModel):
    ...
    title = models.CharField(
        _("Salutation"),
        max_length=5,
        choices=Profile.Salutation.choices,
        default=Profile.Salutation.MR,
    )
        ...
        if self.is_primary:
            ...
            if primary_kin.exists():
                raise ValidationError(
                    {
                        "is_primary": _(
                            "Only one primary next of kin is allowed per profile"
                        )
                    }
                )
    ...
    def __str__(self) -> str:
        return f"{self.first_name} {self.last_name} - Next of Kin for {self.profile.user.fullname}"
